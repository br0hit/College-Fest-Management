...
def create_user():
    if request.method == 'POST':
        ...
        user = User(username=username, email=email, password=password, is_allocated=is_allocated)
        db.session.add(user)
        db.session.commit()
...
def login_participant():
    if request.method == 'POST':
        ...
        if user:
            ...
            return "Participant login failed!"
...
def login_organiser():
    if request.method == 'POST':
        ...
        user = next((user for user in users if user.username == username and user.password == password), None)
        if user:
            return "Organiser login successful!"
        else:
            return "Organiser login failed!"
...
def participant_events():
    ...
    return render_template('participant_events.html', events=events)

@app.route('/participant_events/<event_name>')
def participant_event_details(event_name):
    for event in events_data:
        if event[0] == event_name:
            event_details = {
                "Name": event[0],
    ...
    return render_template('participant_event_details.html', event=event_details)
...
@app.route('/student_events/<event_name>')
def student_event_details(event_name):
    for event in events_data:
        if event[0] == event_name:
            event_details = {
                "Name": event[0],
    ...
    return render_template('student_event_details.html', event=event_details)
...
def organiser_events():
    ...
    return render_template('organiser_events.html', events=events)

@app.route('/organiser_events/<event_name>')
def organiser_event_details(event_name):
    for event in events_data:
        if event[0] == event_name:
            event_details = {
                "Name": event[0],
    ...
    return render_template('organiser_event_details.html', event=event_details)
...
@app.route('/register_as_participant', methods=['POST'])
def register_as_participant():
    event_name = request.json['eventName']
    # Perform any registration logic here
    print("Registration as Participant successful")
    # You can send the event name to another route if needed
    return jsonify({'message': 'Registration as Participant successful', 'eventName': event_name})

@app.route('/register_as_volunteer', methods=['POST'])
def register_as_volunteer():
    event_name = request.json['eventName']
    # Perform any registration logic here
    print("Registration as Volunteer successful")
    # You can send the event name to another route if needed
    return jsonify({'message': 'Registration as Volunteer successful', 'eventName': event_name})
